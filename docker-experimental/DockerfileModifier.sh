...
if [ ! -f ./resources/build_data/vnni ]; then
    ENABLE_VNNI=1
fi

if [ ! -f ./resources/build_data/avx512bf16 ]; then
    ENABLE_VNNI=1
    ENABLE_BF16=1
fi


if [ ! -f ./resources/build_data/amxbf16 ]; then
    ENABLE_VNNI=1
    ENABLE_BF16=1
    ENABLE_AMXBF16=1
fi
...
HEALTHCHECK --interval=30s --timeout=10s --start-period=30s --retries=5 \
  CMD curl -f http://localhost:${VLLM_WEBUI_PORT:-8090}/ || exit 1
